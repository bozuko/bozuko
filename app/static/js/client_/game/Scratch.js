...
Bozuko.client.game.Scratch = Compose( Bozuko.client.game.Abstract, function(){
    ...
    init : function(){
        ...
        Bozuko.each( this.scratchMasks, function(mask){
            var img = new Image();
            img.src = mask;
    ...
    render : function(parent){
        ...
        this.$ct = $('<div class="game scratch-ticket" />')
        ...
        for(var i=0; i<6; i++){
            ...
            self.$prizes[i] = $('<div class="prize" />').appendTo(self.$ct).css({
                left: pos.x+'px',
                top: pos.y+'px'
            });
            
            self.$triggers[i] = $('<div class="target" />').appendTo(self.$ct).css({
                left: pos.x+'px',
                top: pos.y+'px'
            });
            
            var scratch = (function(i){ return function(e){
                self.scratch(i)
            }})(i);
            
            self.$triggers[i][0].addEventListener('touchstart', scratch);
            self.$triggers[i].mousedown(scratch);
            
        }
        
        // add the canvas
        this.$ticket = $('<canvas class="ticket" />').appendTo(this.$ct)
            .attr('width',this.width)
            .attr('height',this.height)
            ;
            
        this.$ticketCtx = this.$ticket[0].getContext('2d');
